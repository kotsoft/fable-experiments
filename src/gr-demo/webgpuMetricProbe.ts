export interface WebGpuMetricProbeResult {
  supported: boolean;
  message: string;
  output?: Float32Array;
  maxAbsDiff?: number;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  let r = ks_radius(p, a);
  let r2 = r * r;
  let scalar = 2.0 * mass * r2 * r / max(r2 * r2 + a * a * p.z * p.z, 1.0e-8);
  let l = ks_l(p, a, r);
  let horizon = mass + sqrt(max(mass * mass - a * a, 0.0));
  outputData[i] = vec4<f32>(r, scalar, dot(l, l), horizon);
}
`;

export async function runWebGpuMetricProbe(
  samples: Float32Array<ArrayBufferLike>,
  expected: Float32Array<ArrayBufferLike>,
): Promise<WebGpuMetricProbeResult> {
  ...
  const module = device.createShaderModule({ code: METRIC_SHADER });
  ...
  pass.dispatchWorkgroups(Math.ceil(inputCopy.length / 4 / 64));
  pass.end();
  ...
  return {
    supported: true,
    message: 'WebGPU metric probe matched CPU reference',
    output: outputCopy,
